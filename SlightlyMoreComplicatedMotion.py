...
class Ball:
    def __init__(self, pos_x = 300, pos_y = 250, vel_x = 0,vel_y = 0, density = 1):
        self.pos_x = pos_x
        self.pos_y = pos_y
        self.vel_x = vel_x
        self.vel_y = vel_y
...
##             Originally tried just absorbing one ball into another. Now will try bouncing.
            if distanceto <= 1 and otherball != self:
    ...
    def ballrender(self):
        pygame.draw.circle(screen_1, self.colour, (int (self.pos_x),int (self.pos_y)), 10, 8)
...
universal_ball_list = ballarc( 10,0.5, 100)
##universal_ball_list = []
